...
export function initImportExport() {
    ...
    exportBtn.addEventListener("click", () => {
        ...
        storageGet<CombinedStorageObject>(defaultCombinedStorageObject).then((result) => {
            const combinedStorageObject = { ...defaultCombinedStorageObject, ...result };
            combinedStorageObject.settings = { ...defaultCombinedStorageObject.settings, ...combinedStorageObject.settings };
            const currentDate = new Date();
            download(
                JSON.stringify(combinedStorageObject),
                `ChannelQuarantine ${currentDate.getFullYear()}-${currentDate.getMonth() + 1}-${currentDate.getDate()}.save`,
                ".save"
            );
        });
    });
...
function loadOldFormat(oldStorageObject: OldStorageObject) {
    ...
    storageGet<CombinedStorageObject>(defaultCombinedStorageObject).then((result) => {
        ...
        if (combinedStorageObject.settings.blockSponsoredTiles === undefined) {
            combinedStorageObject.settings.blockSponsoredTiles = false;
        }
        if (combinedStorageObject.settings.hideShortsShelves === undefined) {
            combinedStorageObject.settings.hideShortsShelves = false;
        }

        // Save data
        storageSet(combinedStorageObject);
...
function loadNewFormat(loadedStorageObject: CombinedStorageObject) {
    ...
    storageGet<CombinedStorageObject>(defaultCombinedStorageObject).then((result) => {
        const storageObject = { ...defaultCombinedStorageObject, ...result };
        storageObject.settings = { ...defaultCombinedStorageObject.settings, ...storageObject.settings };
        ...
        if (storageObject.settings.blockSponsoredTiles === undefined) {
            storageObject.settings.blockSponsoredTiles = false;
        }
        if (storageObject.settings.hideShortsShelves === undefined) {
            storageObject.settings.hideShortsShelves = false;
        }

        // Save data
        storageSet(storageObject);
        setSettingsState(SettingsState.BLOCKED_CHANNELS);
        loadDataFromStorage();
        loadSettingsDataFromStorage();
    });
